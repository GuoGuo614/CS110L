...
impl Grid {
    ...
    fn is_out_bound(&self, row: usize, col: usize) -> bool {
        // usize is unsigned so comparisons like `row < 0` are meaningless.
        // A cell is out of bounds if row >= num_rows or col >= num_cols.
        row >= self.num_rows || col >= self.num_cols
    }

    pub fn get(&self, row: usize, col: usize) -> Option<usize> {
        let val = if self.is_out_bound(row, col) {
            None
        } else {
    ...
    pub fn set(&mut self, row: usize, col: usize, val: usize) -> Result<(), &'static str> {
        if self.is_out_bound(row, col) {
            Err("The location is out of bound!")
        } else {
