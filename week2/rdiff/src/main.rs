...
fn main() {
    ...
    let file_vec1: Vec<String> = read_file_lines(filename1).expect("Invalid filename of args1");
    let file_vec2: Vec<String> = read_file_lines(filename2).expect("Invalid filename of args2");

    let grid = lcs(&file_vec1, &file_vec2);
    print_diff(&grid, &file_vec1, &file_vec2, file_vec1.len(), file_vec2.len());
}
