...
impl OpenFile {
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
        let path_link = format!("/proc/{}/fd/{}", pid, fd);
        let path_info = format!("/proc/{}/fdinfo/{}", pid, fd);

        let link = fs::read_link(path_link).ok()?;
        let name = OpenFile::path_to_name(link.to_str()?);

        let contents = fs::read_to_string(path_info).ok()?;
